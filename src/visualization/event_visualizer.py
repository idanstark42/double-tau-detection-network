...
class EventVisualizer:
  ...
  def density_map (self, show_truth=True, ax=None, output_file=None):
    ...
    self.map([clusters_points, tracks_points], scatter=(truth_points if show_truth else None), ax=ax, output_file=output_file, configs=[{'cmap': 'Blues', 'alpha': 1}, {'cmap': 'Oranges', 'alpha': 0.5}])
  ...
  def momentum_map (self, show_truth=True, ax=None, output_file=None):
    ...
    self.map([cluster_points, track_points], weights=[clusters_momentum, tracks_momentum], scatter=(truth_points if show_truth else None), ax=ax, output_file=output_file, configs=[{'cmap': 'Blues', 'alpha': 1}, {'cmap': 'Oranges', 'alpha': 0.5}])
